...
"""
Messy problem but not too bad to think through once you spend some time on it.

Long problem but satisfying to finish.
"""
...
class Solution:
    def maxCandies(
        ...
        boxes_collected: set[int] = set(box for box in queue)
        boxes_opened: set[int] = set()
        while queue:
            current_box = queue.popleft()
            if current_box in boxes_opened:
                continue

            if status[current_box] == 1:
                # Collect the candy from the box
                res += candies[current_box]
                boxes_opened.add(current_box)

                # Collect any keys that exist
                for key in keys[current_box]:
                    status[key] = 1

                    # If we have a box collected then we can add that box to the queue
                    if key in boxes_collected and key not in boxes_opened:
                        queue.append(key)

                # Add to the queue the boxes in this box
                for inner_box in containedBoxes[current_box]:
                    if inner_box not in boxes_opened:
                        queue.append(inner_box)
                        boxes_collected.add(inner_box)
...
if __name__ == "__main__":
    ...
    cls = Solution()
    ans = cls.maxCandies(**test_case_3)
    print("-----")
    print(ans)
